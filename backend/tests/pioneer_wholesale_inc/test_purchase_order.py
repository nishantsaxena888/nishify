...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"date\": \"2025-01-30T21:51:55.005126\", \"id\": 8268, \"status\": \"evening\", \"vendor_id\": 1858}")
...
def test_update():
    ...
    payload['id'] = 8268
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/8268", json=payload)
...
def test_delete():
    ...
    payload['id'] = 8268
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/8268")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 8268})
    assert response.status_code == 200

def test_eq_status():
    response = httpx.get(BASE_URL, params={'status': 'evening'})
    assert response.status_code == 200

def test_eq_vendor_id():
    response = httpx.get(BASE_URL, params={'vendor_id': 1858})
    assert response.status_code == 200
