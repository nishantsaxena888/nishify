...
def _inject_fk(payload):
    p = dict(payload)
    parent = _mk_parent('salesperson', json.loads('{"email": "community", "id": 1589, "name": "data", "phone": "choose"}'))
...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"address\": \"sound\", \"credit_limit\": 7866.14, \"email\": \"population\", \"id\": 3307, \"name\": \"talk\", \"phone\": \"why\", \"salesperson_id\": 2570}")
...
def test_update():
    ...
    payload['id'] = 3307
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/3307", json=payload)
...
def test_delete():
    ...
    payload['id'] = 3307
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/3307")
...
def test_eq_address():
    response = httpx.get(BASE_URL, params={'address': 'sound'})
    assert response.status_code == 200

def test_eq_credit_limit():
    response = httpx.get(BASE_URL, params={'credit_limit': 7866.14})
    assert response.status_code == 200

def test_eq_email():
    response = httpx.get(BASE_URL, params={'email': 'population'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 3307})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'talk'})
    assert response.status_code == 200

def test_eq_phone():
    response = httpx.get(BASE_URL, params={'phone': 'why'})
    assert response.status_code == 200

def test_eq_salesperson_id():
    response = httpx.get(BASE_URL, params={'salesperson_id': 2570})
    assert response.status_code == 200
