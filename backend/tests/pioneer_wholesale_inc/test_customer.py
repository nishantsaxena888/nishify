...
def _inject_fk(payload):
    p = dict(payload)
    parent = _mk_parent('salesperson', json.loads('{"email": "response", "id": 2356, "name": "price", "phone": "mission"}'))
...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"address\": \"hour\", \"credit_limit\": 7979.82, \"email\": \"again\", \"id\": 9766, \"name\": \"trade\", \"phone\": \"for\", \"salesperson_id\": 3489}")
...
def test_update():
    ...
    payload['id'] = 9766
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/9766", json=payload)
...
def test_delete():
    ...
    payload['id'] = 9766
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/9766")
...
def test_eq_address():
    response = httpx.get(BASE_URL, params={'address': 'hour'})
    assert response.status_code == 200

def test_eq_credit_limit():
    response = httpx.get(BASE_URL, params={'credit_limit': 7979.82})
    assert response.status_code == 200

def test_eq_email():
    response = httpx.get(BASE_URL, params={'email': 'again'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 9766})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'trade'})
    assert response.status_code == 200

def test_eq_phone():
    response = httpx.get(BASE_URL, params={'phone': 'for'})
    assert response.status_code == 200

def test_eq_salesperson_id():
    response = httpx.get(BASE_URL, params={'salesperson_id': 3489})
    assert response.status_code == 200
