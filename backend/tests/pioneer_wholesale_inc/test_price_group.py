...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 2808, \"markup_percent\": 7028.93, \"name\": \"anything\"}")
...
def test_update():
    ...
    payload['id'] = 2808
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/2808", json=payload)
...
def test_delete():
    ...
    payload['id'] = 2808
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/2808")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 2808})
    assert response.status_code == 200

def test_eq_markup_percent():
    response = httpx.get(BASE_URL, params={'markup_percent': 7028.93})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'anything'})
    assert response.status_code == 200
