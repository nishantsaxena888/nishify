...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 5386, \"markup_percent\": 9270.89, \"name\": \"follow\"}")
...
def test_update():
    ...
    payload['id'] = 5386
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/5386", json=payload)
...
def test_delete():
    ...
    payload['id'] = 5386
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/5386")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 5386})
    assert response.status_code == 200

def test_eq_markup_percent():
    response = httpx.get(BASE_URL, params={'markup_percent': 9270.89})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'follow'})
    assert response.status_code == 200
