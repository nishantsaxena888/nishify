...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"discount_percent\": 8857.44, \"id\": 327, \"name\": \"range\", \"terms\": \"process\"}")
...
def test_update():
    ...
    payload['id'] = 327
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/327", json=payload)
...
def test_delete():
    ...
    payload['id'] = 327
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/327")
...
def test_eq_discount_percent():
    response = httpx.get(BASE_URL, params={'discount_percent': 8857.44})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 327})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'range'})
    assert response.status_code == 200

def test_eq_terms():
    response = httpx.get(BASE_URL, params={'terms': 'process'})
    assert response.status_code == 200
