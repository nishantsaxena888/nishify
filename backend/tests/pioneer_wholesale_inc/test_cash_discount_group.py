...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"discount_percent\": 5687.05, \"id\": 4549, \"name\": \"foot\", \"terms\": \"around\"}")
...
def test_update():
    ...
    payload['id'] = 4549
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/4549", json=payload)
...
def test_delete():
    ...
    payload['id'] = 4549
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/4549")
...
def test_eq_discount_percent():
    response = httpx.get(BASE_URL, params={'discount_percent': 5687.05})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 4549})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'foot'})
    assert response.status_code == 200

def test_eq_terms():
    response = httpx.get(BASE_URL, params={'terms': 'around'})
    assert response.status_code == 200
