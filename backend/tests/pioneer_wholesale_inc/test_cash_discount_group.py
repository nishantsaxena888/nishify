...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"discount_percent\": 1564.02, \"id\": 9247, \"name\": \"a\", \"terms\": \"spend\"}")
...
def test_update():
    ...
    payload['id'] = 9247
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/9247", json=payload)
...
def test_delete():
    ...
    payload['id'] = 9247
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/9247")
...
def test_eq_discount_percent():
    response = httpx.get(BASE_URL, params={'discount_percent': 1564.02})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 9247})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'a'})
    assert response.status_code == 200

def test_eq_terms():
    response = httpx.get(BASE_URL, params={'terms': 'spend'})
    assert response.status_code == 200
