...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"exactly\", \"id\": 1171, \"name\": \"manage\"}")
...
def test_update():
    ...
    payload['id'] = 1171
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/1171", json=payload)
...
def test_delete():
    ...
    payload['id'] = 1171
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/1171")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'exactly'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 1171})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'manage'})
    assert response.status_code == 200
