...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"way\", \"id\": 1168, \"name\": \"thank\"}")
...
def test_update():
    ...
    payload['id'] = 1168
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/1168", json=payload)
...
def test_delete():
    ...
    payload['id'] = 1168
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/1168")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'way'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 1168})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'thank'})
    assert response.status_code == 200
