...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"movie\", \"id\": 8094, \"name\": \"despite\"}")
...
def test_update():
    ...
    payload['id'] = 8094
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/8094", json=payload)
...
def test_delete():
    ...
    payload['id'] = 8094
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/8094")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'movie'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 8094})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'despite'})
    assert response.status_code == 200
