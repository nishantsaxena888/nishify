...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"fire\", \"id\": 1488, \"name\": \"thing\"}")
...
def test_update():
    ...
    payload['id'] = 1488
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/1488", json=payload)
...
def test_delete():
    ...
    payload['id'] = 1488
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/1488")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'fire'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 1488})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'thing'})
    assert response.status_code == 200
