...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"animal\", \"id\": 8735, \"name\": \"citizen\"}")
...
def test_update():
    ...
    payload['id'] = 8735
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/8735", json=payload)
...
def test_delete():
    ...
    payload['id'] = 8735
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/8735")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'animal'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 8735})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'citizen'})
    assert response.status_code == 200
