...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"example\", \"id\": 8680, \"name\": \"most\"}")
...
def test_update():
    ...
    payload['id'] = 8680
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/8680", json=payload)
...
def test_delete():
    ...
    payload['id'] = 8680
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/8680")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'example'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 8680})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'most'})
    assert response.status_code == 200
