...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"idea\", \"id\": 7256, \"name\": \"get\"}")
...
def test_update():
    ...
    payload['id'] = 7256
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/7256", json=payload)
...
def test_delete():
    ...
    payload['id'] = 7256
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/7256")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'idea'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 7256})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'get'})
    assert response.status_code == 200
