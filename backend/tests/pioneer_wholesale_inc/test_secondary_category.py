...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"less\", \"id\": 2825, \"name\": \"cover\"}")
...
def test_update():
    ...
    payload['id'] = 2825
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/2825", json=payload)
...
def test_delete():
    ...
    payload['id'] = 2825
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/2825")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'less'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 2825})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'cover'})
    assert response.status_code == 200
