...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"relationship\", \"id\": 9216, \"name\": \"the\"}")
...
def test_update():
    ...
    payload['id'] = 9216
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/9216", json=payload)
...
def test_delete():
    ...
    payload['id'] = 9216
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/9216")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'relationship'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 9216})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'the'})
    assert response.status_code == 200
