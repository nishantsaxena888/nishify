...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"fight\", \"id\": 3244, \"name\": \"enjoy\"}")
...
def test_update():
    ...
    payload['id'] = 3244
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/3244", json=payload)
...
def test_delete():
    ...
    payload['id'] = 3244
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/3244")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'fight'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 3244})
    assert response.status_code == 200
