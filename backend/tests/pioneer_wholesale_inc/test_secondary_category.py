...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"range\", \"id\": 2053, \"name\": \"whatever\"}")
...
def test_update():
    ...
    payload['id'] = 2053
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/2053", json=payload)
...
def test_delete():
    ...
    payload['id'] = 2053
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/2053")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'range'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 2053})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'whatever'})
    assert response.status_code == 200
