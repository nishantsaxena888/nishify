...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"as\", \"id\": 891, \"name\": \"become\"}")
...
def test_update():
    ...
    payload['id'] = 891
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/891", json=payload)
...
def test_delete():
    ...
    payload['id'] = 891
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/891")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'as'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 891})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'become'})
    assert response.status_code == 200
