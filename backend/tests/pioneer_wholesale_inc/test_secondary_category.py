...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"description\": \"professor\", \"id\": 4774, \"name\": \"respond\"}")
...
def test_update():
    ...
    payload['id'] = 4774
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/4774", json=payload)
...
def test_delete():
    ...
    payload['id'] = 4774
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/4774")
...
def test_eq_description():
    response = httpx.get(BASE_URL, params={'description': 'professor'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 4774})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'respond'})
    assert response.status_code == 200
