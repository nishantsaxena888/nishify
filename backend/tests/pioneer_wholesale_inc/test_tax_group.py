...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 1685, \"name\": \"say\", \"tax_percent\": 6604.16}")
...
def test_update():
    ...
    payload['id'] = 1685
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/1685", json=payload)
...
def test_delete():
    ...
    payload['id'] = 1685
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/1685")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 1685})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'say'})
    assert response.status_code == 200

def test_eq_tax_percent():
    response = httpx.get(BASE_URL, params={'tax_percent': 6604.16})
    assert response.status_code == 200
