...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"email\": \"enough\", \"id\": 5807, \"name\": \"because\", \"phone\": \"serious\"}")
...
def test_update():
    ...
    payload['id'] = 5807
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/5807", json=payload)
...
def test_delete():
    ...
    payload['id'] = 5807
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/5807")
...
def test_eq_email():
    response = httpx.get(BASE_URL, params={'email': 'enough'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 5807})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'because'})
    assert response.status_code == 200

def test_eq_phone():
    response = httpx.get(BASE_URL, params={'phone': 'serious'})
    assert response.status_code == 200
