...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"email\": \"example\", \"id\": 9802, \"name\": \"PM\", \"phone\": \"amount\"}")
...
def test_update():
    ...
    payload['id'] = 9802
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/9802", json=payload)
...
def test_delete():
    ...
    payload['id'] = 9802
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/9802")
...
def test_eq_email():
    response = httpx.get(BASE_URL, params={'email': 'example'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 9802})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'PM'})
    assert response.status_code == 200

def test_eq_phone():
    response = httpx.get(BASE_URL, params={'phone': 'amount'})
    assert response.status_code == 200
