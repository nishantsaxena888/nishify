...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"email\": \"during\", \"id\": 7893, \"name\": \"truth\", \"phone\": \"never\"}")
...
def test_update():
    ...
    payload['id'] = 7893
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/7893", json=payload)
...
def test_delete():
    ...
    payload['id'] = 7893
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/7893")
...
def test_eq_email():
    response = httpx.get(BASE_URL, params={'email': 'during'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 7893})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'truth'})
    assert response.status_code == 200

def test_eq_phone():
    response = httpx.get(BASE_URL, params={'phone': 'never'})
    assert response.status_code == 200
