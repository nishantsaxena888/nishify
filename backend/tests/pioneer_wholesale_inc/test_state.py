...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 4793, \"name\": \"different\"}")
...
def test_update():
    ...
    payload['id'] = 4793
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/4793", json=payload)
...
def test_delete():
    ...
    payload['id'] = 4793
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/4793")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 4793})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'different'})
    assert response.status_code == 200
