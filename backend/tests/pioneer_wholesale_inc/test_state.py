...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 3318, \"name\": \"media\"}")
...
def test_update():
    ...
    payload['id'] = 3318
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/3318", json=payload)
...
def test_delete():
    ...
    payload['id'] = 3318
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/3318")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 3318})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'media'})
    assert response.status_code == 200
