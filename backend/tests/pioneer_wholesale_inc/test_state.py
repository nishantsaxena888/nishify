...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 659, \"name\": \"act\"}")
...
def test_update():
    ...
    payload['id'] = 659
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/659", json=payload)
...
def test_delete():
    ...
    payload['id'] = 659
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/659")
...
# eq filters
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 659})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'act'})
    assert response.status_code == 200
