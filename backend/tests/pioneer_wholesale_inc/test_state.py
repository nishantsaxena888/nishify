...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 433, \"name\": \"as\"}")
...
def test_update():
    ...
    payload['id'] = 433
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/433", json=payload)
...
def test_delete():
    ...
    payload['id'] = 433
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/433")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 433})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'as'})
    assert response.status_code == 200
