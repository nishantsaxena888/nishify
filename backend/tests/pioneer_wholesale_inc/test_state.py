...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"id\": 7729, \"name\": \"red\"}")
...
def test_update():
    ...
    payload['id'] = 7729
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/7729", json=payload)
...
def test_delete():
    ...
    payload['id'] = 7729
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/7729")
...
def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 7729})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'red'})
    assert response.status_code == 200
