...
def _inject_fk(payload):
    p = dict(payload)
    parent = _mk_parent('invoice', json.loads('{"customer_id": 7663, "date": "2025-06-08T10:55:07.924611", "id": 7380, "status": "building"}'))
...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"invoice_id\": 4355, \"item_id\": 1401, \"price\": 2054.03, \"quantity\": 3059}")
...
def test_eq_invoice_id():
    response = httpx.get(BASE_URL, params={'invoice_id': 4355})
    assert response.status_code == 200

def test_eq_item_id():
    response = httpx.get(BASE_URL, params={'item_id': 1401})
    assert response.status_code == 200

def test_eq_price():
    response = httpx.get(BASE_URL, params={'price': 2054.03})
    assert response.status_code == 200

def test_eq_quantity():
    response = httpx.get(BASE_URL, params={'quantity': 3059})
    assert response.status_code == 200
