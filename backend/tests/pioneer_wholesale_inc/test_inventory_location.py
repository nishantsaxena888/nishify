...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"address\": \"a\", \"id\": 7968, \"name\": \"result\"}")
...
def test_update():
    ...
    payload['id'] = 7968
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/7968", json=payload)
...
def test_delete():
    ...
    payload['id'] = 7968
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/7968")
...
def test_eq_address():
    response = httpx.get(BASE_URL, params={'address': 'a'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 7968})
    assert response.status_code == 200
