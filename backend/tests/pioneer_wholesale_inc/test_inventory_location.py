...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"address\": \"much\", \"id\": 4267, \"name\": \"seek\"}")
...
def test_update():
    ...
    payload['id'] = 4267
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/4267", json=payload)
...
def test_delete():
    ...
    payload['id'] = 4267
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/4267")
...
def test_eq_address():
    response = httpx.get(BASE_URL, params={'address': 'much'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 4267})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'seek'})
    assert response.status_code == 200
