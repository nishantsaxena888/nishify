...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"address\": \"same\", \"id\": 1106, \"name\": \"cause\"}")
...
def test_update():
    ...
    payload['id'] = 1106
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/1106", json=payload)
...
def test_delete():
    ...
    payload['id'] = 1106
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/1106")
...
def test_eq_address():
    response = httpx.get(BASE_URL, params={'address': 'same'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 1106})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'cause'})
    assert response.status_code == 200
