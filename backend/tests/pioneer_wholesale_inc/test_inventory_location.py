...
def test_create():
    global CREATED_ID
    payload = json.loads("{\"address\": \"already\", \"id\": 4333, \"name\": \"result\"}")
...
def test_update():
    ...
    payload['id'] = 4333
    httpx.post(BASE_URL, json=payload)
    response = httpx.put(f"{BASE_URL}/4333", json=payload)
...
def test_delete():
    ...
    payload['id'] = 4333
    httpx.post(BASE_URL, json=payload)
    response = httpx.delete(f"{BASE_URL}/4333")
...
# eq filters
def test_eq_address():
    response = httpx.get(BASE_URL, params={'address': 'already'})
    assert response.status_code == 200

def test_eq_id():
    response = httpx.get(BASE_URL, params={'id': 4333})
    assert response.status_code == 200

def test_eq_name():
    response = httpx.get(BASE_URL, params={'name': 'result'})
    assert response.status_code == 200
